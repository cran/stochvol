...
is proposed. The paper at hand and the corresponding package \pkg{stochvol} \citep{r:sto} for \proglang{R} \citep{r:r} are crafted to cope with the latter problem: the apparent lack of ready-to-use packages for efficiently estimating SV models.
...
\begin{figure}[!htp]
\begin{center}
 \includegraphics[width=.96\textwidth]{article-usd4}
...
\begin{figure}[!htp]
\begin{center}
 \includegraphics[width=.96\textwidth]{article-usd5}
...
@
\item Finally, summarize and visualize the posterior draws:
<<>>=
colMeans(draws1)
@
...
\begin{figure}[!ht]
\begin{center}
\includegraphics[width=.98\textwidth]{article-homo}
...
@
\item Finally, summarize and visualize (some) posterior draws:
<<eval=FALSE>>=
colMeans(draws2[,4:8])
@
...
\begin{figure}[!ht]
 \begin{center}
\includegraphics[width=.95\textwidth]{article-hetero}
...
It is worth noting that for other exchange rates contained in \code{exrates}, a qualitatively similar picture emerges. For an overview of cumulative log predictive likelihoods, see Table~\ref{exrates}. Detailed results about these currencies are available from the author.
...
\begin{table}[!ht]
 ...
 Currency & SV & GARCH & homoskedastic \\
  ...
  \hline
 \end{tabular}
 \caption{Final cumulative predictive log likelihoods for AR(1) models with different error assumptions for the logarithm of several EUR exchange rates included in \pkg{stochvol}'s dataset \code{exrates}. SV is strongly favored in all cases.}
 \label{exrates}
\end{table}
